...
def get_posts():
    ...
    try:
        ...
        if username_receive == '':
            posts = list(db.posts.find({}).sort('date', -1).limit(20)) # untuk menampilkan 20 postingan terakhir dan urut berdasar 'date'
        else:
            posts = list(db.posts.find({'username': username_receive}).sort('date', -1).limit(20)) # untuk menampilkan 20 postingan terakhir dan urut berdasar 'date'
        for post in posts:
